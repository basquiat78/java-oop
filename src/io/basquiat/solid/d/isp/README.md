...
하지만 정작 필요없는 녀석이라 지우고 싶은 마음이 너무나 간절하지만 IDE님께서 결코 허락하지 않으신다.

위 예제는 정확하게 말하자면 ISP를 위반한 것이다.
...
한편으로는 이런 원칙에 입각하게 되면 불어나는 인터페이스는 어떻게 되는건가???     

또한 독특하게도 이 원칙은 SRP와 많이 연관해서 말하는데 

이런 말들을 많이 듣게 된다.

<h3>
SRP에 입각한 설계를 하면 ISP가 성립이 되는가?
</h3>
...
게시판이라는 것은 가만히 보면 read, write, modify, delete를 수행한다.

그런데 delete의 경우에는 관리자만 가능하다고 한다면 사용자 관점에서 보면 이것은 ISP에 위배되는 모습을 보인다.

하지만 게시판 입장에서는 SRP가 성립되고 있다는 것이다.